...
本地/非 HFS 部署下，`DAP_DATA_DIR` 或 `DAP_PERSIST_DIR` 应与 `DAP_DB_PATH` / `DAP_BUSINESS_DB_PATH` 保持同一持久化目录。HFS v2 thin wrapper 使用标准 `/data` Storage Bucket mount，并将 `DAP_DATA_DIR` 默认固定为 `/data/data-agent-platform`；显式数据库、任务目录或 HF cache 路径必须仍在该目录内，越界或 `/data` 不可写会直接失败，不会回退到 repo 内或 `/tmp`。这些路径属于 wrapper 的本地部署控制，不作为普通 Space Variable 分发。
