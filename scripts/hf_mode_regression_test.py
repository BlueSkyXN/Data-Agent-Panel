...
ops_headers = {"X-Ops-Token": os.environ["OPS_TOKEN"]} if os.getenv("OPS_TOKEN") else {}
...
for path in ops_paths:
    r = client.get(path, headers=ops_headers)
    expected = 200 if os.getenv("OPS_TOKEN") else 503
    assert r.status_code == expected, (path, r.status_code, r.text)

if os.getenv("OPS_TOKEN"):
    ...
    cookie_response = client.get("/_ops/", params={"token": os.environ["OPS_TOKEN"]}, follow_redirects=False)
    ...
    assert os.environ["OPS_TOKEN"] not in dashboard.text
    ...
    secure_cookie_response = secure_client.get(
        "/_ops/",
        params={"token": os.environ["OPS_TOKEN"]},
